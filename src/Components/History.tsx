...
const History = ({ data }) => {
    ...
    return (
        <section style={{ margin: "20px 0px" }}>
            <h2>History</h2>
            {!data || data.length == 0 ?
                <section className="otherContainer"> <p>No history yet.</p></section> :
                data.map((item, inx) => {
                    const output = item.wrongWordsData ? item.wrongWordsData.map((el) => (
                    ...
                    return (
                        <section key={inx} className="otherContainer">
                            ...
                            <div className="result">
                                ...
                                <div>
                                    <span>Wrong</span> <span className="wrong small">{item.wrongWords}</span>
                                </div>
                            </div>
                            <div className="wrongWords">
                                <div className="h4 wrongWords__title">WRONG WORDS</div>
                                <div className="wrongWords__div">{output.length > 0 ? output : <div style={{textAlign:"center",width:"100%"}}>No Wrong Words</div>}</div>
                            </div>
                        </section>
                    )
                })}
